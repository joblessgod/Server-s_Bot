...
client.on("interactionCreate", (interaction) => {
  ...
  if (interaction.commandName == "hey") {
    interaction.reply(`heellooooo <@${interaction.user.id}>`);
  }
});
  ...
  if (welcomeChannelId) {
    ...
    if (welcomeChannel) {
      const welcomeEmbed = new EmbedBuilder()
        ...
        .setTitle("🎉 Welcome!")
        .setDescription(
          `Welcome to the server, ${member.user.tag}! We're glad to have you here!`
        )
...
client.on("messageCreate", async (message) => {
  ...
  if (userLevel.messages >= 10) {
    // Example threshold for leveling up
    userLevel.level += 1;
    ...
    const levelUpEmbed = new EmbedBuilder()
      ...
      .setTitle("🎉 Level Up!")
      .setDescription(
        `${message.author}, you've leveled up to level ${userLevel.level}! Keep it going!`
      );
  ...
  if (command === "play") {
    ...
    if (userLevel) {
      const levelEmbed = new EmbedBuilder()
        ...
        .setTitle("📊 Your Level")
        .setDescription(
          `You are currently level **${userLevel.level}** with **${userLevel.messages}** messages.`
        );
    ...
    const helpEmbed = new EmbedBuilder()
      .setColor("#1E90FF")
      .setTitle("🛠️ Bot Commands").setDescription(`
**🎵 Music Commands:**
...
`);
    ...
    const rulesEmbed = new EmbedBuilder()
      .setColor("#FF4500")
      .setTitle("📜 Server Rules").setDescription(`
...
`);
    ...
    if (!channel) {
      return message.reply(
        "❗ Please mention a valid channel to set as the welcome channel!"
      );
    }
    ...
    const sellEmbed = new EmbedBuilder()
      ...
      .setTitle("💸 Sell Item")
      .setDescription(
        `You've sold **${item}** and earned **${earnings}** money!`
      );
...
- **Role 2**: $1000
    ...
    if (robberySuccess) {
      ...
      const robEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `You've successfully robbed **${member.user.tag}** and earned **${robAmount}** money!`
        );
    ...
    if (!member || isNaN(amount)) {
      return message.reply(
        "❗ Please mention a user and specify an amount to give."
      );
    }
    ...
    if (targetEco) {
      ...
      const giveEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `You have given **${amount}** money to **${member.user.tag}**.`
        );
    ...
    if (!member || isNaN(amount)) {
      return message.reply(
        "❗ Please mention a user and specify an amount to pay."
      );
    }
    ...
    const payEmbed = new EmbedBuilder()
      ...
      .setDescription(
        `You have paid **${amount}** money to **${member.user.tag}**.`
      );
    ...
    const ticketEmbed = new EmbedBuilder()
      ...
      .setDescription(
        `Your support ticket has been created: ${ticketChannel}. An administrator will assist you shortly.`
      );
